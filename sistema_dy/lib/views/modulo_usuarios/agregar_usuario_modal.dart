...
class _AgregarUsuarioDialogState extends State<AgregarUsuarioDialog> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          // Filtrar solo los empleados con estado "activo" (case-insensitive)
          empleados = empleados.where((empleado) =>
              (empleado["estado"]?.toString().toLowerCase() ?? "activo") ==
              "activo").toList();
          if (empleados.isNotEmpty) {
            selectedEmpleadoId = empleados.first["id"].toString();
            cargo = empleados.first["cargo"] ?? 'Administrador';
            _cargoController.text = cargo;
          }
        });
      } else {
  ...
  Future<void> _submit() async {
    ...
    if (password.length < 8) {
      _showAlertDialog("Error", "La contraseña debe tener al menos 8 caracteres");
      return;
    }
    ...
    final empleado = empleados.firstWhere(
        (emp) => emp["id"].toString() == selectedEmpleadoId);
                  ...
                  children: [
                    if (empleados.isEmpty)
                      const Padding(
                        ...
                        child: Text("No hay empleados disponibles para registrar"),
                      )
                    else ...[
                      DropdownButtonFormField<String>(
                        ...
                        onChanged: (value) {
                          setState(() {
                            ...
                            final empleado = empleados.firstWhere((emp) => emp["id"].toString() == value);
                      ...
                      TextFormField(
                        ...
                        onChanged: (value) => setState(() => usuario = value),
                      ),
                      const SizedBox(height: 16),
                      TextFormField(
                      ...
                      DropdownButtonFormField<String>(
                        value: tipoCuenta,
                        decoration: _inputDecoration("Tipo de Cuenta", Icons.people_alt),
                        icon: const Icon(Icons.arrow_drop_down, color: Color(0xFF3C3C3C)),
                        ...
                        onChanged: (value) => setState(() => tipoCuenta = value!),
                      ),
                      const SizedBox(height: 16),
                      // Campo Cargo de solo lectura
                      TextFormField(
